...
const int nvertices = 50 ;
                                                                /*update here */
float points[400] ;// no of vertices times 2(x,y)* 2(ring)*2 ... (x8)
...
void genpoint(float* points, unsigned int* ind ){
  for (int i = 0; i<(m); i = i+8){                              
    points[i]   = 0.4 * cos(2*M_PI/nvertices*i/8) ;
    points[i+1] = 0.4 * sin(2*M_PI/nvertices*i/8) ;
    points[i+4] = 0.5 * cos(2*M_PI/nvertices*i/8) ;
    points[i+5] = 0.5 * sin(2*M_PI/nvertices*i/8) ;
    points[i+2] = points[i] + 0.5 ;
    points[i+3] = points[i+1] + 0.5 ;
    points[i+6] = points[i+4] + 0.5 ;
    points[i+7] = points[i+5] + 0.5 ;
  }
...
int main(int argc, char** argv)
{
  ...
  window = glfwCreateWindow(640, 640, "Textured Ring", NULL, NULL);
  if (!window)
    {
      glfwTerminate();
      return -1;
